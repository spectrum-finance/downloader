...
import repositories.pg.{PostgresTransactor, doobieLogging}
...
//      downloaderTrans <- PostgresTransactor.make[InitF]("downloader-backend-tracker", configs.downloaderPg)
      ...
      implicit0(state: StateRepository[RunF])        <- Resource.eval(StateRepository.create[InitF, RunF])
      implicit0(resolver: WeightResolver[RunF])      <- Resource.eval(WeightResolver.make[InitF, RunF](configs.resolver))
      implicit0(program: DownloaderProgram[StreamF]) <- Resource.eval(DownloaderProgram.make[InitF, RunF, StreamF](configs.resolver))
